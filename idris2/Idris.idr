...
mutual
  ...
  Tnary' : {n : Nat} -> SpecialList l -> Vect n Type -> Vect (2 + k) (TDefR n) ->
           (Type -> Type -> Type) -> Type
  Tnary' sp tvars [x, y]              c =
    c (assert_total $ Ty' sp tvars x) (assert_total $ Ty' sp tvars y)
  Tnary' sp tvars (x :: y :: z :: zs) c =
    c (assert_total $ Ty' sp tvars x) (assert_total $ Tnary' sp tvars (y :: z :: zs) c)
  ...
  Ty' sp tvars (TMu m) with (depLookup sp (TMu m))
    ...
    Ty' sp tvars (TMu m) | Just (MkDPair _ (MkDPair constr prf)) = ApplyVect constr tvars
  ...
  Ty' sp tvars (TApp (TName _ def) xs) with (depLookup sp def)
    ...
    -- we found.
    Ty' sp tvars (TApp (TName _ def) xs) | Just (MkDPair arity (MkDPair constr prf)) =
        let args = map (assert_total $ Ty' sp tvars) xs in ApplyVect constr args

Ty : {n : Nat} -> Vect n Type -> TDefR n -> Type
Ty = Ty' []
...
AlphaTy : {n : Nat} -> SpecialList l -> TDefR n -> TypeConstructor n
AlphaTy sp tdef = ReverseVect (flip (Ty' sp) tdef)

NatSum : {0 f : Nat -> Type} -> Vect n (k : Nat ** f k) -> Nat
NatSum [] = Z
...
convertTy : {def : TDefR k} -> Ty' [] v (TApp (TName n def) xs) -> Ty' [] v (def `ap` xs)
...
data Printer : Vect n Type -> Type where
  Nil : Printer []
  (::) : (a -> String) -> Printer as -> Printer (a :: as)


mutual

  showMu : {n : Nat} -> {types : Vect n Type}
        -> Printer types
        -> (td : TDef' (S n) b)
        -> Mu' [] types td -> String
  showMu tvars td (Inn x) =
    let prt : ((Mu' [] (types) td) -> String) = assert_total $ showMu tvars td
     in parens' (assert_total $ showTy ((prt)::tvars) td x)

  printIndex : {n : Nat} -> {0 types : Vect (S n) Type} ->
               (idx : Fin (S n)) -> Printer types -> Ty' [] types (TVar idx) -> String
  printIndex FZ (print :: ps) x = print x
  printIndex (FS FZ) (_ :: print :: ps) x = print x
  printIndex (FS (FS y)) (_ :: ps) x = printIndex (FS y) ps x

  printRef : {n : Nat} -> {0 types : Vect (S n) Type} ->
               (idx : Fin (S n)) -> Printer types -> Ty' [] types (RRef idx) -> String
  printRef FZ (print :: ps) x = print x
  printRef (FS FZ) (_ :: print :: ps) x = print x
  printRef (FS (FS y)) (_ :: ps) x = printRef (FS y) ps x
  ...
  showTy : {n : Nat} -> {types : Vect n Type}
        -> Printer types
        -> (td : TDefR n)
        -> (x : Ty' [] types td)
        -> String
  ...
  showTy tvars (TVar idx)           x         = printIndex idx tvars x
  showTy tvars (RRef idx)           x         = printRef idx tvars x
  showTy tvars (TApp (TName name def) xs)  x
    = assert_total $ showTy tvars (def `ap` xs) (convertTy x)
  showTy tvars  (TProd xs) x
    = parens $ concat $ List.intersperse ", " (showProd xs x)
    where showProd : (ys : Vect (2 + k) (TDefR n))
                  -> Tnary' [] types ys Pair -> List String
          showProd [a, b]        (x, y) = (showTy tvars a x)::[showTy tvars b y]
          showProd (a::b::c::ys) (x, y) = (showTy tvars a x)::showProd (b::c::ys) y

(td : TDef' 1 a) => Show ( Mu' sp {l=Z} [] td) where
  show y {sp=[]} = showMu [] td y

---------------------------------------------------------------------------------
-- Type Equality                                                               --
---------------------------------------------------------------------------------
-- Note: Equality on Types only works when the specialisation context is empty --
--------------------------------------- -----------------------------------------

    {-
mutual

  eqMu : {n : Nat} -> (tvars : Vect n (a : Type ** a -> a -> Bool)) -> (td : TDefR (S n)) ->
  ...
  eqTy : {n : Nat} -> (tvars : Vect n (a : Type ** a -> a -> Bool)) -> (td : TDefR n) ->
