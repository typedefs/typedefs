...
alias : {n : Nat} -> String -> TNamed' n b -> TNamed' n b
...
ap (TApp (TName _ def) xs) args = assert_total $ def `ap` (map (flip ap args) xs)
ap (RRef i)    args  = Vect.index i args
...
mutual
  ...
  heteroEq {n} {m} tn tm = case (cmp n m) of
    (CmpLT y) => tm == (weakenTDef tn _ (lteAddRight n)) -- or should this be `False`?
    (CmpGT x) => tn == (weakenTDef tm _ (lteAddRight m)) -- or should this be `False`?
    (CmpEQ)   => tn == tm
...
powN : {n : Nat} -> Nat -> TNamed' n a -> TDef' n a
powN m tn = pow m (wrap tn)
...
mutual
  ...
  showTDef (FRef n)    = n
  showTDef (RRef n)    = curly $ show $ toNat n
  ...
  showNTDefs : Vect k (String, TDef' n a) -> String
  showNTDefs []          = ""
