...
tsumInj : {n, k : Nat} -> {v : Bool} -> {a : Vect (2+n) (TDef' k v)} -> {b : Vect (2+n) (TDef' k v)} ->
          TSum {k=n} a = TSum {k=n} b -> a = b
tsumInj Refl = Refl

tprodInj : {n, k : Nat} -> {a : Vect (2+n) (TDef' k v)} -> {b : Vect (2+n) (TDef' k v)} ->
           TProd {k=n} a = TProd {k=n} b -> a = b
tprodInj Refl = Refl

tvarInj : {n : Nat} -> {i, j : Fin (S n)} -> TVar i = TVar j -> i = j
tvarInj Refl = Refl
...
mutual
  [vectTDef] DecEq (Vect n (TDef' n v)) where
    decEq [] [] = Yes Refl
    decEq (x :: xs) (y :: ys) = ?whatever

  public export
  DecEq (TDef' n v) where
    decEq T0              T0                   = Yes Refl
    decEq T1              T1                   = Yes Refl
    decEq (TSum {k=k} xs)   (TSum {k=k2} xs')    with (decEq k k2)
      decEq (TSum {k} xs) (TSum {k} xs')         | Yes Refl with (assert_total $ decEq @{vectTDef} xs xs')
