...
roundtrip1 : (td : TDef 0) -> Ty [] td -> Maybe (Ty [] td) -- Maybe ((Ty [] td), Bytes)
roundtrip1 td x =
 case deserializeBinaryClosed td $ serializeBinaryClosed td x of
  Nothing => Nothing
  Just (x', _) => Just x'
...
--      (Just (Inn (Right (1, Inn (Right (2, Inn (Left ())))))))
  ...
  describe "Binary serialisation/deserialisation" $ do
    ...
    it "round1 unit" $ roundtrip1 T1 () `shouldBe` (Just ())

    it "round1 sum" $ roundtrip1 (TSum [T1, T1]) (Right ())  `shouldBe` (Just (Right ()))

    it "round1 prod" $ roundtrip1 (TProd [T1, T1]) ((), ())  `shouldBe` (Just ((), ()))

    it "round1 mu base" $ roundtrip1 (TMu [("Nil", T1), ("Cons", TProd [(TMu [("Z", T1), ("S", TVar 0)]), TVar 0])]) (Inn (Left ()))
      `shouldBe` (Just (Inn (Left ())))
