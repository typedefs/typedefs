...
custLookup : DecEq t => {w : t} -> {f, g : t -> Type}
          -> List (v : t ** (f v, g v)) -> f w -> Eq (f w) => Maybe (v ** g v)
...
custLookup ((n ** (key, val)) :: xs) item {w} with (decEq n w)
  ...
  custLookup ((n ** (key, val)) :: xs) item {w=n} | Yes Refl =
    if item == key then Just (n ** val) -- index & key match, return value and its index
...
||| Get the length from a vector using its type
private length' : Vect n a -> Nat
length' _ {n} = n
...
mutual
  ...
  Ty' : SpecialList -> Vect n Type ->  TDefR n -> Type
  Ty' rep tvars T0             = Void
  Ty' rep tvars T1             = Unit
  Ty' rep tvars (TSum xs) {n}  = Tnary' rep tvars xs Either
  Ty' rep tvars (TProd xs) {n} = Tnary' rep tvars xs Pair
  Ty' rep tvars (TVar v)       = Vect.index v tvars
  Ty' rep tvars (TMu m)        = Mu' rep tvars (args m)
  Ty' rep tvars (RRef i)       = Vect.index i tvars

  -- For application we first make a lookup in our mapping from TDef to Type constructors
  Ty' rep tvars (TApp (TName _ def) xs) with (custLookup rep def)
    -- If we can't find anything, simply apply the type normally
    Ty' rep tvars (TApp (TName _ def) xs) | Nothing = assert_total $ Ty' rep tvars $ def `ap` xs
    -- If we find a match, check the length of the arguments match the arity of the type constructor
    -- we found.
    Ty' rep tvars (TApp (TName _ def) xs) | Just (v ** constr) with (decEq v (length' xs))
      -- If the arities don't match, just apply as normal
      Ty' rep tvars (TApp (TName _ def) xs) | Just (v ** constr) | No contra =
        assert_total $ Ty' rep tvars $ def `ap` xs
      -- If the arities match, recursively convert the vector of arguments into a vector of `Type`
      -- and apply it to the type constructor we found
      Ty' rep tvars (TApp (TName _ def) xs ) | Just (v ** constr) | Yes prf =
        let args = map (assert_total $ Ty' rep tvars ) xs in ApplyVect args (rewrite sym prf in constr)
